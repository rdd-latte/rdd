...
// Every download action on this custom rehost serves this exact file.
// Keep the ZIP in the same directory as index.html.
...
async function downloadFixedZip() {
    ...
    try {
        ...
        if (!response.ok) {
            throw new Error(`HTTP ${response.status}. Make sure ${FIXED_ZIP_FILE} is beside index.html.`);
        }
...
async function main() {
    ...
    // Permanent/query links also download the same locally hosted ZIP.
    await downloadFixedZip();
    return;
